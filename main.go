			...
			Div(Css("bg-white rounded shadow-lg border flex flex-col overflow-hidden"),
				...
				Div(Css("px-6 py-3 border-t"),
					Div(Css("flex justify-end"),
						Button(Css("bg-gray-700 text-gray-100 rounded px-4 py-2 mr-1"), OnClick(cancel),
							Text("Cancel"),
						),
						Button(Css("bg-blue-600 text-gray-200 rounded px-4 py-2 disabled:opacity-50"), Disabled(err() != ""), OnClick(save),
							Text("Save"),
						),
					),
				),
...
func Index(c *RenderContext) UI {
	...
	return Col(Css("bg-black w-full h-full"),
		...
		Row(
			Button(Css("bg-gray-300 text-gray-900 rounded hover:bg-gray-200 px-4 py-2 focus:outline-none"), OnClick(toggleTimer),
				IfElse(!running().(bool), Text("Start"), Text("Stop")),
			),
			Button(Css("bg-gray-300 text-gray-900 rounded hover:bg-gray-200 px-4 py-2 focus:outline-none ml-10"), OnClick(openModal),
				Text("Set Timer"),
			),
		),
